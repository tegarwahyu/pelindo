	...
	function aksi_login(){
		...
		if($cek > 0)
		{
			foreach($cek as $id)
			{
				...
				{
					//untuk PIC
					$data_session = array('id_users'=> $id->ID_USERS,'position'=> $id->POSITION,'nama' => $username, 'divisi' => $id->DIVISION, 'expertise'=> $id->EXPERTISE, 'role' => $id->ROLE, 'status' => "login");
					$this->session->set_userdata('akses','4');
					$this->session->set_userdata($data_session);
					$this->session->set_flashdata('message', '<div class="alert alert-success">Selamat datang, Anda berhasil login.</div>');
					redirect(base_url("pic/pic/dasboard"));
				}
				else
				{
